-- Production assumptions belong to the bought-out scope allocation: one
-- subcontract can cover several cost codes, and several subcontractors can
-- share one cost code. Keeping the inputs here avoids pretending the GC knows
-- the subcontractor's internal payroll while still turning a lump-sum buyout
-- into a repeatable production benchmark.
