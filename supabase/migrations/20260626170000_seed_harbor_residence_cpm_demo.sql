-- Backfill the live Harbor Residence demo projects with a real CPM activity plan.
-- The app seed covers new workspaces, but existing demo projects need a data migration
-- so direct project links show the teaching schedule immediately after publish.

WITH demo_projects AS (
  SELECT id
  FROM public.projects
  WHERE lower(name) = 'harbor residence'
),
