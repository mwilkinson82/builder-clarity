...
CREATE INDEX IF NOT EXISTS estimate_plan_sets_estimate_idx
  ON public.estimate_plan_sets(estimate_id, updated_at DESC);

CREATE INDEX IF NOT EXISTS estimate_plan_sets_org_idx
  ON public.estimate_plan_sets(organization_id, updated_at DESC);

DROP TRIGGER IF EXISTS estimate_plan_sets_set_updated_at ON public.estimate_plan_sets;
CREATE TRIGGER estimate_plan_sets_set_updated_at
  BEFORE UPDATE ON public.estimate_plan_sets
...
CREATE TABLE IF NOT EXISTS public.estimate_plan_sheets (
  ...
  CONSTRAINT estimate_plan_sheets_name_or_number CHECK (
    length(trim(sheet_number)) > 0 OR length(trim(sheet_name)) > 0
  )
);

CREATE INDEX IF NOT EXISTS estimate_plan_sheets_set_idx
  ON public.estimate_plan_sheets(plan_set_id, sort_order);

CREATE INDEX IF NOT EXISTS estimate_plan_sheets_estimate_idx
  ON public.estimate_plan_sheets(estimate_id, sort_order);

DROP TRIGGER IF EXISTS estimate_plan_sheets_set_updated_at ON public.estimate_plan_sheets;
CREATE TRIGGER estimate_plan_sheets_set_updated_at
  BEFORE UPDATE ON public.estimate_plan_sheets
  FOR EACH ROW EXECUTE FUNCTION public.tg_set_updated_at();
...
CREATE INDEX IF NOT EXISTS estimate_takeoff_measurements_estimate_idx
  ON public.estimate_takeoff_measurements(estimate_id, updated_at DESC);

CREATE INDEX IF NOT EXISTS estimate_takeoff_measurements_sheet_idx
  ON public.estimate_takeoff_measurements(plan_sheet_id, created_at);

CREATE INDEX IF NOT EXISTS estimate_takeoff_measurements_line_idx
  ON public.estimate_takeoff_measurements(estimate_line_item_id)
...
CREATE POLICY estimate_takeoff_measurements_org_select
  ...
  USING (
    EXISTS (
      SELECT 1
      FROM public.estimates e
      WHERE e.id = estimate_takeoff_measurements.estimate_id
...
CREATE POLICY estimate_takeoff_measurements_org_insert
  ...
  WITH CHECK (
    ...
    AND EXISTS (
      ...
      JOIN public.estimates e ON e.id = ps.estimate_id
...
CREATE POLICY plan_room_storage_team_read
  ...
  USING (
    ...
    AND EXISTS (
      SELECT 1
      FROM public.estimates e
      WHERE e.id = public.storage_estimate_id(name)
        AND public.is_org_member(e.organization_id)
    )
  );
