-- P0 final connector ACL seal.
--
-- The Lovable read-only database connector restores sandbox_exec grants after
-- a query completes. Migration 01400 proved the revocation works, and the full
-- live catalog/integrity audit then reopened those connector grants. This must
-- be the final managed database operation in the cutover. Do not call Lovable
-- query_database after it; production application traffic does not use this
-- operational role.
--
-- This migration changes no function bodies and writes no business data. It
-- removes sandbox_exec from the complete Auth/authorization surface and fails
-- atomically if any explicit or inherited EXECUTE capability remains.
