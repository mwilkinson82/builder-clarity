-- Plan Room RLS upload fix: use estimate-level access checks instead of storage-only path check.
...
CREATE POLICY estimate_plan_sets_org_select ON public.estimate_plan_sets
...
CREATE POLICY estimate_plan_sets_org_insert ON public.estimate_plan_sets
...
CREATE POLICY estimate_plan_sets_org_update ON public.estimate_plan_sets
  FOR UPDATE TO authenticated USING (public.can_manage_estimate(estimate_id)) WITH CHECK (public.can_manage_estimate(estimate_id));

DROP POLICY IF EXISTS estimate_plan_sets_org_delete ON public.estimate_plan_sets;
CREATE POLICY estimate_plan_sets_org_delete ON public.estimate_plan_sets
...
CREATE POLICY estimate_plan_sheets_org_select ON public.estimate_plan_sheets
  FOR SELECT TO authenticated USING (public.can_read_estimate(estimate_id));

DROP POLICY IF EXISTS estimate_plan_sheets_org_insert ON public.estimate_plan_sheets;
CREATE POLICY estimate_plan_sheets_org_insert ON public.estimate_plan_sheets
  FOR INSERT TO authenticated WITH CHECK (
    public.can_manage_estimate(estimate_id)
    AND EXISTS (SELECT 1 FROM public.estimate_plan_sets ps WHERE ps.id = estimate_plan_sheets.plan_set_id AND ps.estimate_id = estimate_plan_sheets.estimate_id)
  );

DROP POLICY IF EXISTS estimate_plan_sheets_org_update ON public.estimate_plan_sheets;
CREATE POLICY estimate_plan_sheets_org_update ON public.estimate_plan_sheets
  FOR UPDATE TO authenticated USING (public.can_manage_estimate(estimate_id)) WITH CHECK (
    public.can_manage_estimate(estimate_id)
    AND EXISTS (SELECT 1 FROM public.estimate_plan_sets ps WHERE ps.id = estimate_plan_sheets.plan_set_id AND ps.estimate_id = estimate_plan_sheets.estimate_id)
  );

DROP POLICY IF EXISTS estimate_plan_sheets_org_delete ON public.estimate_plan_sheets;
CREATE POLICY estimate_plan_sheets_org_delete ON public.estimate_plan_sheets
  FOR DELETE TO authenticated USING (public.can_manage_estimate(estimate_id));

DROP POLICY IF EXISTS estimate_takeoff_measurements_org_select ON public.estimate_takeoff_measurements;
CREATE POLICY estimate_takeoff_measurements_org_select ON public.estimate_takeoff_measurements
  FOR SELECT TO authenticated USING (public.can_read_estimate(estimate_id));

DROP POLICY IF EXISTS estimate_takeoff_measurements_org_insert ON public.estimate_takeoff_measurements;
CREATE POLICY estimate_takeoff_measurements_org_insert ON public.estimate_takeoff_measurements
  FOR INSERT TO authenticated WITH CHECK (
    public.can_manage_estimate(estimate_id)
    AND (created_by IS NULL OR created_by = (SELECT auth.uid()))
    AND EXISTS (SELECT 1 FROM public.estimate_plan_sheets ps WHERE ps.id = estimate_takeoff_measurements.plan_sheet_id AND ps.estimate_id = estimate_takeoff_measurements.estimate_id)
  );

DROP POLICY IF EXISTS estimate_takeoff_measurements_org_update ON public.estimate_takeoff_measurements;
CREATE POLICY estimate_takeoff_measurements_org_update ON public.estimate_takeoff_measurements
  FOR UPDATE TO authenticated USING (public.can_manage_estimate(estimate_id)) WITH CHECK (
    public.can_manage_estimate(estimate_id)
    AND EXISTS (SELECT 1 FROM public.estimate_plan_sheets ps WHERE ps.id = estimate_takeoff_measurements.plan_sheet_id AND ps.estimate_id = estimate_takeoff_measurements.estimate_id)
  );
...
CREATE POLICY estimate_takeoff_measurements_org_delete ON public.estimate_takeoff_measurements
  FOR DELETE TO authenticated USING (public.can_manage_estimate(estimate_id));

DROP POLICY IF EXISTS plan_room_storage_team_read ON storage.objects;
CREATE POLICY plan_room_storage_team_read ON storage.objects
  FOR SELECT TO authenticated USING (bucket_id = 'plan-room' AND public.can_read_estimate(public.storage_estimate_id(name)));

DROP POLICY IF EXISTS plan_room_storage_team_insert ON storage.objects;
CREATE POLICY plan_room_storage_team_insert ON storage.objects
  FOR INSERT TO authenticated WITH CHECK (bucket_id = 'plan-room' AND public.can_manage_estimate(public.storage_estimate_id(name)));

DROP POLICY IF EXISTS plan_room_storage_team_update ON storage.objects;
CREATE POLICY plan_room_storage_team_update ON storage.objects
  FOR UPDATE TO authenticated
  USING (bucket_id = 'plan-room' AND public.can_manage_estimate(public.storage_estimate_id(name)))
  WITH CHECK (bucket_id = 'plan-room' AND public.can_manage_estimate(public.storage_estimate_id(name)));

DROP POLICY IF EXISTS plan_room_storage_team_delete ON storage.objects;
CREATE POLICY plan_room_storage_team_delete ON storage.objects
  FOR DELETE TO authenticated USING (bucket_id = 'plan-room' AND public.can_manage_estimate(public.storage_estimate_id(name)));
