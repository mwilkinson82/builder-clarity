...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'organizations_stripe_connect_status_check'
  ) THEN
    ALTER TABLE public.organizations
...
COMMENT ON COLUMN public.subscription_plans.stripe_price_id IS
  'Stripe Price ID used by Checkout Sessions for subscription signup once paid Overwatch plans are enabled.';

COMMENT ON COLUMN public.subscription_plans.checkout_enabled IS
  'When false, the plan can be shown for readiness without sending users into paid Stripe checkout.';

COMMENT ON COLUMN public.organizations.contractor_circle_grant IS
  'Current rollout grant. Keep Contractor Circle users working even when commercial plan meters are visible.';
...
COMMENT ON COLUMN public.organizations.stripe_connect_account_id IS
  'Reserved for the connected Stripe account that will receive client invoice payments.';

COMMENT ON COLUMN public.billing_invoices.payment_url IS
  'Hosted payment URL, typically generated by a Stripe Checkout Session for invoice payment.';

COMMENT ON COLUMN public.billing_invoices.payment_enabled IS
  'Controls whether this invoice can show a client-facing online payment action.';

COMMENT ON COLUMN public.payment_ledger.stripe_payment_intent_id IS
  'Stripe PaymentIntent captured from a hosted payment flow; manual payments leave this blank.';
