...
DO $$ BEGIN
  CREATE TYPE public.project_member_role AS ENUM ('owner', 'manager', 'editor', 'viewer');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
CREATE TABLE IF NOT EXISTS public.organizations (
  ...
  daily_report_limit_per_month integer NOT NULL DEFAULT 1000,
  contractor_circle_grant boolean NOT NULL DEFAULT true,
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'profiles_default_organization_fkey'
  ) THEN
    ALTER TABLE public.profiles
      ADD CONSTRAINT profiles_default_organization_fkey
...
CREATE TABLE IF NOT EXISTS public.organization_invites (
  ...
  organization_id uuid NOT NULL REFERENCES public.organizations(id) ON DELETE CASCADE,
  email text NOT NULL,
  role public.account_role NOT NULL DEFAULT 'project_manager',
  ...
  SELECT auth.uid() IS NOT NULL
    AND EXISTS (
      ...
      WHERE m.organization_id = p_org_id
        AND m.user_id = auth.uid()
        AND m.status = 'active'
        AND m.role IN ('owner', 'admin', 'executive', 'project_manager')
    );
$$;
...
BEGIN
  ...
  FOR v_invite IN
    ...
    WHERE lower(i.email) = lower(coalesce(p_email, ''))
...
CREATE OR REPLACE FUNCTION public.on_auth_user_account_created()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
CREATE OR REPLACE FUNCTION public.tg_projects_owner_membership()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  IF NEW.organization_id IS NOT NULL AND NEW.owner_id IS NOT NULL THEN
    INSERT INTO public.organization_memberships (organization_id, user_id, role, status)
    VALUES (NEW.organization_id, NEW.owner_id, 'owner', 'active')
    ON CONFLICT (organization_id, user_id) DO NOTHING;
...
REVOKE ALL ON FUNCTION public.on_auth_user_account_created() FROM PUBLIC;
REVOKE ALL ON FUNCTION public.tg_projects_ensure_organization() FROM PUBLIC;
REVOKE ALL ON FUNCTION public.tg_projects_owner_membership() FROM PUBLIC;
...
CREATE TRIGGER on_auth_user_account_created
  ...
  FOR EACH ROW EXECUTE FUNCTION public.on_auth_user_account_created();

DROP TRIGGER IF EXISTS projects_ensure_organization ON public.projects;
CREATE TRIGGER projects_ensure_organization
  BEFORE INSERT ON public.projects
  FOR EACH ROW EXECUTE FUNCTION public.tg_projects_ensure_organization();

DROP TRIGGER IF EXISTS projects_owner_membership ON public.projects;
CREATE TRIGGER projects_owner_membership
  AFTER INSERT ON public.projects
  FOR EACH ROW EXECUTE FUNCTION public.tg_projects_owner_membership();
...
BEGIN
  ...
  LOOP
    INSERT INTO public.organization_memberships (organization_id, user_id, role, status)
    VALUES (v_project.organization_id, v_project.owner_id, 'owner', 'active')
    ON CONFLICT (organization_id, user_id) DO NOTHING;

    INSERT INTO public.project_memberships (project_id, user_id, role, status)
    VALUES (v_project.id, v_project.owner_id, 'owner', 'active')
    ON CONFLICT (project_id, user_id) DO NOTHING;
  END LOOP;
END $$;
...
CREATE POLICY organizations_create_own
  ON public.organizations
  FOR INSERT
  TO authenticated
...
CREATE POLICY project_memberships_read
...
CREATE POLICY project_memberships_manage_insert
  ON public.project_memberships
  FOR INSERT
  TO authenticated
...
CREATE POLICY project_memberships_manage_update
  ON public.project_memberships
  FOR UPDATE
  TO authenticated
...
CREATE POLICY project_memberships_manage_delete
  ON public.project_memberships
  FOR DELETE
  TO authenticated
  USING (public.can_manage_project(project_id));
...
CREATE POLICY projects_team_update
  ON public.projects
  FOR UPDATE
  TO authenticated
...
CREATE POLICY projects_team_delete
  ON public.projects
  FOR DELETE
  TO authenticated
  USING (public.can_manage_project(id));
...
CREATE POLICY exposures_team_select ON public.exposures
...
CREATE POLICY exposures_team_insert ON public.exposures
...
CREATE POLICY exposures_team_update ON public.exposures
...
CREATE POLICY exposures_team_delete ON public.exposures
...
CREATE POLICY change_orders_team_select ON public.change_orders
  FOR SELECT TO authenticated USING (public.can_read_project(project_id));
DROP POLICY IF EXISTS change_orders_team_insert ON public.change_orders;
CREATE POLICY change_orders_team_insert ON public.change_orders
  FOR INSERT TO authenticated WITH CHECK (public.can_manage_project(project_id));
DROP POLICY IF EXISTS change_orders_team_update ON public.change_orders;
CREATE POLICY change_orders_team_update ON public.change_orders
  FOR UPDATE TO authenticated USING (public.can_manage_project(project_id))
  WITH CHECK (public.can_manage_project(project_id));
DROP POLICY IF EXISTS change_orders_team_delete ON public.change_orders;
CREATE POLICY change_orders_team_delete ON public.change_orders
  FOR DELETE TO authenticated USING (public.can_manage_project(project_id));
...
CREATE POLICY decisions_team_select ON public.decisions
  FOR SELECT TO authenticated USING (public.can_read_project(project_id));
...
CREATE POLICY decisions_team_update ON public.decisions
  FOR UPDATE TO authenticated USING (public.can_manage_project(project_id))
  WITH CHECK (public.can_manage_project(project_id));
DROP POLICY IF EXISTS decisions_team_delete ON public.decisions;
CREATE POLICY decisions_team_delete ON public.decisions
  FOR DELETE TO authenticated USING (public.can_manage_project(project_id));

DROP POLICY IF EXISTS reviews_team_select ON public.reviews;
CREATE POLICY reviews_team_select ON public.reviews
  FOR SELECT TO authenticated USING (public.can_read_project(project_id));
DROP POLICY IF EXISTS reviews_team_insert ON public.reviews;
CREATE POLICY reviews_team_insert ON public.reviews
  FOR INSERT TO authenticated WITH CHECK (public.can_manage_project(project_id));
DROP POLICY IF EXISTS reviews_team_update ON public.reviews;
CREATE POLICY reviews_team_update ON public.reviews
  FOR UPDATE TO authenticated USING (public.can_manage_project(project_id))
  WITH CHECK (public.can_manage_project(project_id));
DROP POLICY IF EXISTS reviews_team_delete ON public.reviews;
CREATE POLICY reviews_team_delete ON public.reviews
  FOR DELETE TO authenticated USING (public.can_manage_project(project_id));

DROP POLICY IF EXISTS schedule_milestones_team_select ON public.schedule_milestones;
CREATE POLICY schedule_milestones_team_select ON public.schedule_milestones
  FOR SELECT TO authenticated USING (public.can_read_project(project_id));
...
CREATE POLICY schedule_risks_team_select ON public.schedule_risks
  FOR SELECT TO authenticated USING (public.can_read_project(project_id));
...
CREATE POLICY billing_applications_team_insert ON public.billing_applications
  FOR INSERT TO authenticated WITH CHECK (public.can_manage_project(project_id));
DROP POLICY IF EXISTS billing_applications_team_update ON public.billing_applications;
CREATE POLICY billing_applications_team_update ON public.billing_applications
  FOR UPDATE TO authenticated USING (public.can_manage_project(project_id))
  WITH CHECK (public.can_manage_project(project_id));
DROP POLICY IF EXISTS billing_applications_team_delete ON public.billing_applications;
CREATE POLICY billing_applications_team_delete ON public.billing_applications
  FOR DELETE TO authenticated USING (public.can_manage_project(project_id));

DROP POLICY IF EXISTS daily_reports_team_select ON public.daily_reports;
CREATE POLICY daily_reports_team_select ON public.daily_reports
  FOR SELECT TO authenticated USING (public.can_read_project(project_id));
DROP POLICY IF EXISTS daily_reports_team_insert ON public.daily_reports;
CREATE POLICY daily_reports_team_insert ON public.daily_reports
  FOR INSERT TO authenticated WITH CHECK (public.can_manage_project(project_id));
DROP POLICY IF EXISTS daily_reports_team_update ON public.daily_reports;
CREATE POLICY daily_reports_team_update ON public.daily_reports
  FOR UPDATE TO authenticated USING (public.can_manage_project(project_id))
  WITH CHECK (public.can_manage_project(project_id));
DROP POLICY IF EXISTS daily_reports_team_delete ON public.daily_reports;
CREATE POLICY daily_reports_team_delete ON public.daily_reports
  FOR DELETE TO authenticated USING (public.can_manage_project(project_id));

DROP POLICY IF EXISTS daily_reports_storage_team_read ON storage.objects;
CREATE POLICY daily_reports_storage_team_read
  ON storage.objects
  FOR SELECT
  TO authenticated
  USING (
...
DROP POLICY IF EXISTS daily_reports_storage_team_insert ON storage.objects;
CREATE POLICY daily_reports_storage_team_insert
  ON storage.objects
  FOR INSERT
  TO authenticated
  WITH CHECK (
    bucket_id = 'daily-reports'
    AND public.can_manage_project(public.storage_project_id(name))
  );

DROP POLICY IF EXISTS daily_reports_storage_team_update ON storage.objects;
CREATE POLICY daily_reports_storage_team_update
  ON storage.objects
  FOR UPDATE
  TO authenticated
  USING (
...
DROP POLICY IF EXISTS daily_reports_storage_team_delete ON storage.objects;
CREATE POLICY daily_reports_storage_team_delete
  ON storage.objects
  FOR DELETE
  TO authenticated
  USING (
