...
--      + a private 'subcontract-docs' storage bucket (mirrors daily-reports:
--      files live in storage, the row records the path; access gated by project
--      ownership via the first path segment = project id).
...
-- Access gated by project ownership: the first folder segment of the object
-- name is the project id (path = <projectId>/<subcontractId>/<file>). Mirrors
-- the daily-reports storage policies exactly.
