...
-- Preserve the founder-approved Harbor sample in the organization where the
-- reusable 15-line learning estimate already exists. Other organizations are
-- seeded through the application after this migration lands.
...
-- Restore the approved $1,606,137 golden total when this known legacy sample
-- still carries the original grading allowance. The guarded update is a no-op
-- everywhere else.
...
-- Existing project-linked demos remain useful working data, but their names
-- must not imply that they are the protected product sample.
UPDATE public.estimates estimate
...
-- Plan Room and other estimate-scoped write paths already call this helper.
-- Canonical samples remain readable while every mutation is rejected.
