...
SELECT
  ...
  b.submitted_date,
  b.due_date,
  COALESCE(b.amount_billed, 0),
  COALESCE(b.retainage, 0),
  ...
  CASE
    WHEN COALESCE(b.paid_to_date, 0) >= GREATEST(0, COALESCE(b.amount_billed, 0) - COALESCE(b.retainage, 0))
      AND GREATEST(0, COALESCE(b.amount_billed, 0) - COALESCE(b.retainage, 0)) > 0
      THEN 'paid'
...
WHERE NOT EXISTS (
  SELECT 1
  FROM public.billing_invoices i
  WHERE i.billing_application_id = b.id
);
