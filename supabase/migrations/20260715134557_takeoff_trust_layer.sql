...
--
-- Quantity is no longer just a number supplied by a browser. Each takeoff
-- records how it was calculated, which scale revision it used, and whether it
-- is safe to roll into the estimate. Scale changes invalidate dependent
-- linear/area work atomically. Counts remain scale-independent.
...
-- Existing count work is deterministic and needs no scale. Existing linear
-- and area work predates calculation provenance, so it must be reviewed once
-- instead of being silently represented as verified.
...
BEGIN
  ...
  IF scale_changed THEN
    ...
    -- A changed measurement basis must be checked again against a known
    -- dimension, even if a stale client sent the old verification timestamp.
...
-- Re-assert Data API privileges after adding the trust columns. RLS remains
-- the authorization boundary for authenticated access.
