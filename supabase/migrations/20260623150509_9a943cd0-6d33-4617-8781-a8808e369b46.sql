...
REVOKE ALL ON FUNCTION public.record_client_change_order_decision(uuid, public.client_approval_decision, text, text) FROM PUBLIC;
...
GRANT EXECUTE ON FUNCTION public.record_client_change_order_decision(uuid, public.client_approval_decision, text, text) TO authenticated;
...
CREATE POLICY client_contacts_org_read ON public.client_contacts FOR SELECT TO authenticated USING (public.is_org_member(organization_id));

DROP POLICY IF EXISTS client_contacts_org_insert ON public.client_contacts;
CREATE POLICY client_contacts_org_insert ON public.client_contacts FOR INSERT TO authenticated WITH CHECK (public.can_create_project_in_org(organization_id));

DROP POLICY IF EXISTS client_contacts_org_update ON public.client_contacts;
CREATE POLICY client_contacts_org_update ON public.client_contacts FOR UPDATE TO authenticated USING (public.can_create_project_in_org(organization_id)) WITH CHECK (public.can_create_project_in_org(organization_id));

DROP POLICY IF EXISTS client_contacts_org_delete ON public.client_contacts;
CREATE POLICY client_contacts_org_delete ON public.client_contacts FOR DELETE TO authenticated USING (public.can_create_project_in_org(organization_id));

DROP POLICY IF EXISTS project_client_access_internal_or_client_read ON public.project_client_access;
CREATE POLICY project_client_access_internal_or_client_read ON public.project_client_access FOR SELECT TO authenticated USING (
  public.can_read_project(project_id)
  OR (
    auth.uid() IS NOT NULL
    AND status IN ('pending', 'active')
    AND (client_user_id = auth.uid() OR lower(email) = lower(coalesce(auth.jwt() ->> 'email', '')))
  )
);

DROP POLICY IF EXISTS project_client_access_project_insert ON public.project_client_access;
CREATE POLICY project_client_access_project_insert ON public.project_client_access FOR INSERT TO authenticated WITH CHECK (public.can_manage_project(project_id));

DROP POLICY IF EXISTS project_client_access_project_update ON public.project_client_access;
CREATE POLICY project_client_access_project_update ON public.project_client_access FOR UPDATE TO authenticated USING (public.can_manage_project(project_id)) WITH CHECK (public.can_manage_project(project_id));

DROP POLICY IF EXISTS project_client_access_project_delete ON public.project_client_access;
CREATE POLICY project_client_access_project_delete ON public.project_client_access FOR DELETE TO authenticated USING (public.can_manage_project(project_id));

DROP POLICY IF EXISTS change_order_approvals_project_read ON public.change_order_approvals;
CREATE POLICY change_order_approvals_project_read ON public.change_order_approvals FOR SELECT TO authenticated USING (public.can_read_project(project_id) OR public.can_read_client_project(project_id));

DROP POLICY IF EXISTS change_order_approvals_client_insert ON public.change_order_approvals;
CREATE POLICY change_order_approvals_client_insert ON public.change_order_approvals FOR INSERT TO authenticated WITH CHECK (
  public.can_approve_client_change_order(change_order_id)
  AND EXISTS (SELECT 1 FROM public.change_orders co WHERE co.id = change_order_id AND co.project_id = project_id)
);

DROP POLICY IF EXISTS change_orders_client_select ON public.change_orders;
CREATE POLICY change_orders_client_select ON public.change_orders FOR SELECT TO authenticated USING (client_visible = true AND public.can_read_client_project(project_id));

DROP POLICY IF EXISTS projects_client_select ON public.projects;
CREATE POLICY projects_client_select ON public.projects FOR SELECT TO authenticated USING (public.can_read_client_project(id));

DROP POLICY IF EXISTS daily_reports_client_select ON public.daily_reports;
CREATE POLICY daily_reports_client_select ON public.daily_reports FOR SELECT TO authenticated USING (client_visible = true AND public.can_read_client_project(project_id));

DROP POLICY IF EXISTS daily_reports_storage_client_read ON storage.objects;
CREATE POLICY daily_reports_storage_client_read ON storage.objects FOR SELECT TO authenticated USING (
