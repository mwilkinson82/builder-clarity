...
| **Job cost** | [`JobCostReport.tsx`](../../src/components/reports/JobCostReport.tsx) | `listPortfolioJobCost` | The project **Budget tab** (`computeBudgetLedger`). Pick a job → per cost code: budget, actual, committed, projected (EAC), over/(under) budget, at risk, contingency, % used. |
